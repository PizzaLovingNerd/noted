...
const AlbumCard = ({ album, onClick }: { album: Album; onClick: (album: Album) => void }) => (
  <div 
  ...
  >
    <div className="w-36 h-36 bg-gray-200 rounded-lg overflow-hidden shadow-lg mb-2 relative group border border-black/5">
      {album.artworkUrl100 ? (
        ...
        <div className="w-full h-full flex items-center justify-center bg-gray-300 text-gray-500 text-xs">No Cover</div>
      )}
    </div>
    <p className="text-sm font-bold truncate text-gray-900 leading-tight">{album.collectionName}</p>
    <p className="text-xs text-gray-600 truncate">{album.artistName}</p>
  </div>
);

const HomeScreen: React.FC<HomeScreenProps> = ({ onAlbumClick, onReviewClick }) => {
        ...
        if (mounted) {
            ...
            if (trendingData.length > 0) {
                setFriendReviews([
                {
                    ...
                    content: 'The production on this is absolutely insane. Layers upon layers.',
                    rating: 5,
                    author: 'Tyler',
                    date: '2h ago'
                },
                {
                    ...
                    content: 'I did not expect this level of lyricism in 2025. Unmatched.',
                    rating: 4,
                    author: 'Sarah',
                    date: '5h ago'
                },
  ...
  return (
    <div className="space-y-8 pb-32 pt-2">
      {/* Trending Section */}
      <section>
        <h2 className="text-xl font-bold px-4 mb-4 font-serif italic text-black">Trending Albums</h2>
        <div className="flex overflow-x-auto px-4 pb-2 no-scrollbar">
          {loading && trending.length === 0 ? (
            [1,2,3].map(i => <div key={i} className="w-36 h-36 bg-gray-200 rounded-lg mr-4 flex-shrink-0 animate-pulse" />)
          ) : (
             trending.map((album, idx) => (
                <AlbumCard key={`${album.collectionId}-${idx}`} album={album} onClick={onAlbumClick} />
             ))
          )}
        </div>
      </section>
      ...
      <section>
        <h2 className="text-xl font-bold px-4 mb-4 font-serif italic text-black">Best of 2025</h2>
        <div className="flex overflow-x-auto px-4 pb-2 no-scrollbar">
      ...
      <section className="px-4">
        <h2 className="text-xl font-bold mb-4 font-serif italic text-black">Friend Reviews</h2>
          ...
          {friendReviews.map((review, idx) => (
            ...
            >
              ...
              <div className="ml-4 flex-1 min-w-0">
                ...
                <p className="text-xs font-bold text-gray-800 truncate">{review.album.collectionName}</p>
                <p className="text-xs text-gray-600 line-clamp-2 mt-1">{review.content}</p>
              </div>
            </div>
          ))}
        </div>
