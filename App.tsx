...
import { HomeIcon, SearchIcon, PlusIcon, UserIcon, ArrowLeftIcon } from './components/Icons';
...
const App = () => {
  ...
  const [searchContext, setSearchContext] = useState<{title: string, query?: string}>({ title: 'Top Albums' });
            ...
            const aiReviews = [
                {
                    ...
                    content: 'The experimental production on this album pushes the boundaries of the genre. Rocky creates a psychedelic soundscape that feels both nostalgic and futuristic.',
                    rating: 4
                },
                {
                    title: 'The reunion we needed',
                    content: 'Pusha T and Malice sound hungrier than ever. The beats are gritty, the flows are impeccable, and the chemistry is undeniable.',
                    rating: 5
                },
                {
                    title: 'A defining moment',
                    content: 'Maximalist production at its finest. Every track feels like a movie. The features are perfectly utilized.',
                    rating: 5
                }
            ];
  ...
  const renderContent = () => {
    switch (currentScreen) {
      case 'home':
        return <HomeScreen onAlbumClick={handleAlbumClick} onReviewClick={handleReviewClick} />;
      case 'search':
        return <SearchScreen onGenreClick={handleGenreClick} onSearch={handleSearchSubmit} />;
      case 'albumList':
        return (
          <AlbumListScreen 
        ...
        return (
            <div className="px-4 pb-24 pt-4">
                <div className="flex items-center mb-6">
                    <button onClick={goBack} className="mr-3"><ArrowLeftIcon /></button>
                    <h2 className="text-2xl font-bold font-serif italic text-black">My Reviews</h2>
                </div>
                <div className="space-y-4">
                    {myReviews.map(review => (
                        ...
                        <div className="ml-3 flex-1 min-w-0">
                            <h4 className="font-bold text-gray-900 text-sm truncate">{review.album.collectionName}</h4>
                            <div className="flex items-center gap-1 text-xs text-yellow-500 mb-1">
                                <span className="font-bold text-black mr-1">{review.rating}.0</span>
      ...
      <div className="w-full max-w-md h-[100dvh] max-h-[100dvh] bg-[#DCEFFB] relative overflow-hidden shadow-2xl sm:rounded-3xl sm:h-[850px] flex flex-col">
        ...
        {(currentScreen === 'home' || currentScreen === 'search') && (
          <header className="px-5 pt-8 pb-2 flex justify-between items-center z-10 relative pt-[max(2rem,env(safe-area-inset-top))]">
            ...
            >
              <span className="font-medium text-sm text-black/80">Jordan</span>
        ...
        {showBottomNav && (
          <nav className="absolute bottom-0 left-0 right-0 min-h-[5rem] pt-4 pb-[max(1rem,env(safe-area-inset-bottom))] bg-white border-t border-gray-100 flex items-center justify-between px-10 z-20">
             ...
             <button onClick={handleSearchClick} className="p-2 flex flex-col items-center gap-1 group">
               <SearchIcon className={`w-7 h-7 ${currentScreen === 'search' ? 'text-black stroke-[3px]' : 'text-gray-400 group-hover:text-black'}`} />
             </button>
          </nav>
        )}
      </div>
